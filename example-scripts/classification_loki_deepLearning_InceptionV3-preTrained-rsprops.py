...
imgDir    = basePath + 'Trainingsdatensatz_preprocessed/'
trainTsv  = basePath + 'train.tsv'
validTsv  = basePath + 'val.tsv'
testTsv   = basePath + 'test.tsv'
whitelist = basePath + 'whitelist.txt'

saveDir   = basePath + 'results/InceptionV3-preTrained-rsprops/'
...
# Read target list.
    with open(tsv) as f:
        reader = csv.reader(f, dialect=csv.unix_dialect, delimiter='\t')
        target = []
        imgId = []
        next(reader)
        i = 0
...
callback_list = [time_callback, earlyStopping_callback, modelCheckpoint_callback]
